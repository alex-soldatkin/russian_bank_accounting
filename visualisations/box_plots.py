# %%
"""
Box-plot + outlier layer (by quarter) with background 'dropout' bubbles
-------------------------------------------------------------------------------
This script mirrors the style and data plumbing of the *journey bands* plot,
except it produces **box-plots grouped by quarter** and overlays **outlier
points** (IQR rule) using a dedicated layer. Behind each box (per quarter), it
adds a translucent **bubble whose size encodes how many banks stopped being
present in the dataset in that quarter** (i.e., their last observation falls in
that quarter).

It is inspired by the Lets-Plot cookbook example
`stat_boxplot_outlier.ipynb`. If `stat_boxplot_outlier` is available in your
Lets-Plot build, this script will use it; otherwise it **computes outliers in
Pandas** and draws them with `geom_point`.

Inputs & assumptions
- Parquet: data/HOORRAAH_final_banking_indicators_preprocessed.parquet
- Columns required: DT (timestamp), REGN (bank id), total_assets, and
  the variables listed below (subset is fine).

Output
- HTML pages in visualisations/html (paged by facets) named
  `box_quarter_outliers_page<N>.html`.

Tuning knobs
- Edit the **PARAMETERS** block for variables, faceting, alphas, sizes, winsor
  clipping, etc.
"""
...
from lets_plot import (
    ggplot, aes, geom_boxplot, geom_point, geom_line, facet_wrap,
    labs, ggsize, ggsave, theme, element_text, scale_size, scale_x_discrete
)

# Optional: use stat_boxplot_outlier if present
try:
    from lets_plot import stat_boxplot_outlier as STAT_BOX_OUTLIER
    HAVE_STAT_BOX_OUTLIER = True
except Exception:
    STAT_BOX_OUTLIER = None
    HAVE_STAT_BOX_OUTLIER = False
...
# Variables to consider (auto-filtered to those present)
PREFERRED_VARS = [
    # Ratios / %
...
OUTLIER_ALPHA      = 0.10
OUTLIER_SIZE       = 2
OUTLIER_COLOR      = 'grey20'  # dark grey

# Background dropout bubble styling
BUBBLE_MIN_SIZE    = 2.0
BUBBLE_MAX_SIZE    = 18.0
BUBBLE_ALPHA       = 0.22   # low alpha so boxes/outliers remain legible
BUBBLE_COLOR       = 'red'
...
def compute_outlier_mask(values: pd.Series) -> pd.Series:
    """Return boolean mask of points flagged as outliers by 1.5×IQR rule per group."""
...
# Precompute QUARTER keys and dropout counts per quarter
DF['quarter'] = DF['DT'].dt.to_period('Q')
DF['quarter_str'] = DF['quarter'].astype(str)  # e.g., '2017Q1'

# Banks that exit in a given quarter = last available quarter per REGN
if 'REGN' not in DF.columns:
    raise ValueError('Expected column REGN (bank id)')
...
# ------------------------------------------------------------------
# Iterate pages of variables
# ------------------------------------------------------------------
for page_idx, var_chunk in enumerate(pages, start=1):
    # Build long frame for the current chunk
    ...
    # Transform per variable to a visualization scale similar to journey bands
    ...
    # Optional clipping to control extreme y scales
    ...
    # Compute per (variable, quarter) medians (for bubble y placement) & outliers
    med = (
        long_viz.groupby(['variable','facet','quarter_str'])['value_viz']
                .median().rename('y_med').reset_index()
    )

    # Dropout bubbles: replicate counts across facets/variables and join y_med for placement
    bubble_df = (
    ...
    # Compute outliers per (variable, quarter)
    if HAVE_STAT_BOX_OUTLIER:
        # We'll delegate outlier detection to Lets-Plot stat layer below
        outliers_df = pd.DataFrame(columns=['facet','quarter_str','value_viz'])
    else:
        def flag_group(g: pd.DataFrame) -> pd.DataFrame:
            m = compute_outlier_mask(g['value_viz'])
            gg = g.loc[m, ['facet','quarter_str','value_viz']].copy()
            return gg
        outliers_df = (
            long_viz.groupby(['variable','facet','quarter_str'], group_keys=False)
                    .apply(flag_group)
                    .reset_index(drop=True)
        )

    # Prepare x-ordering by chronological quarter
    # Keeping a stable order avoids alphabetical 2019Q1, 2019Q2, ...
    all_q = (
    ...
    # Background bubbles first (beneath boxes/outliers)
    ...
    # Box-plots by quarter (hide built-in outliers; we'll draw them ourselves)
    base = base + geom_boxplot(
        aes(x='quarter_str', y='value_viz', group='quarter_str'),
        data=long_viz,
        alpha=BOX_ALPHA,
        width=BOX_WIDTH,
        outlier_size=0,  # hide built-in outliers; we'll add a dedicated layer
    )

    # Dedicated outlier layer
    if HAVE_STAT_BOX_OUTLIER:
        # Use Lets-Plot stat with control over aesthetics
        base = base + STAT_BOX_OUTLIER(
            aes(x='quarter_str', y='value_viz'),
            data=long_viz,
            color=OUTLIER_COLOR,
            size=OUTLIER_SIZE,
            alpha=OUTLIER_ALPHA,
        ...
        # Manual outlier points computed above
    ...
    plot = (
        ...
        + labs(x='Quarter', y='Value', title='Quarterly Box-Plots with Outliers and Dropout Bubbles',
    ...
    # Save
    out_name = f"box_quarter_outliers_page{page_idx}.html"
