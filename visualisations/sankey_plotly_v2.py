...
# Decide the last year with actual data (>0 w_raw once computed)
# Compute w_raw preliminarily (using indicator_ma/raw as placeholder)
pre = pd.to_numeric(SNAP['indicator_ma'].where(SNAP['indicator_ma'].notna(), SNAP['indicator_raw']), errors='coerce')
SNAP['_pre_w_raw'] = np.clip(pre, 0, None)

# Iteratively drop trailing years with zero total
def year_total(y: int) -> float:
    return float(SNAP.loc[SNAP['year'] == y, '_pre_w_raw'].sum())

while len(YEARS) > 1 and year_total(YEARS[-1]) <= 0:
    dropped = YEARS.pop()
      ...
      function resize(){
        ...
        if (gd){
          ...
          if (window.Plotly && gd.data) {
            Plotly.relayout(gd, {autosize:true, width: window.innerWidth, height: window.innerHeight});
