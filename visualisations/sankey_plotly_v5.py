...
# Year-end snapshot + fallback to raw (non-last years)
...
SNAP['indicator_final'] = SNAP['indicator_ma']  # no special RAW override per user request

KIND = decide_kind(INDICATOR_COL)
print(f"[diag] transform kind for {INDICATOR_COL}: {KIND}")

# ------------------------------
# Build year grid — then DROP any trailing empty years
# ------------------------------
...
# ------------------------------
# Build step flows (exits always target the same year’s Exit node)
# ------------------------------
links_rows = []
for i in range(len(YEARS) - 1):
    ...
    # Exit flows: present at y0, missing at y1 -> go to Exit at **y0**
    ...
    if not gone.empty:
        ...
        agx['level_from'] = y0
        agx['level_to']   = y0   # <- keep exits in the same year’s column
        agx['flow_type']  = 'exit'
...
EPS_EXIT = (dx * 0.0005) if ncol > 1 else 0.0005  # tiny nudge within the same column
...
for year in YEARS:
    for cat in cat_order:
        ...
        ys.append(0.5)  # temp; will be set by band layout
        node_colors.append(NODE_COLORS.get(cat, 'rgba(150,150,150,0.85)'))

# --- Deterministic band layout: identical y for a category across all years
...
sankey = go.Sankey(
    arrangement='perpendicular',  # respect provided x/y, no auto re-layout
...
# --- Full-viewport HTML + SVG export helper ---
post_script = (
    """
    (function(){
      ...
      function resize(){
        ...
        if (gd){
          ...
          if (window.Plotly && gd.data) {
            ...
            // keep current x/y on resize for belt-and-braces stability
